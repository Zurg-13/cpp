...
/**
 * @brief Запись в массив.
 */
void TwoDimRD::write_state_to_array(double *data, int idx) {

    const auto &a = this->ta[idx];
    typename MatrixXXd::Index rows = a.rows(), cols = a.cols();
    int size = rows*cols;

    std::cout
        << "| size:" << size
        << ", cell:" << sizeof(typename MatrixXXd::Scalar)
        << std::endl;

    double *d = (double*)a.data();
    for(int i=0; i<size; i++) { data[i] = d[i]; }

}// write_state_to_array
